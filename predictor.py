...
def main():
    ...
    st.header("Ask questions about the election and predict vote percentage")
    ...
    main_parties_support = st.number_input("How many Main Parties that support", min_value=0, value=1)
    adult_voters = st.slider("No. of Adult voters(%)", 0, 100, 50)
    youth_voters = st.slider("No. Youth voters(%)", 0, 100, 30)
    new_voters = st.slider("New Voters(%)", 0, 100, 20)
    inflation_rate = st.slider("Inflation Rate(%)", 0.0, 20.0, 5.0)
    unemployment_rate = st.slider("Unemployment Rate (%)", 0.0, 20.0, 5.0)
    coverage_districts = st.number_input("No. of Coverage Districts", min_value=1, value=10)
    election_spending = st.number_input("Spending on Election (millions)", min_value=0.0, value=1.0)
